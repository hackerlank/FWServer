 ...
 */
static const char* s_nethandshakekey = "hello myworld!";
static const char* s_checkpack = "U\n";
...
static int
lgetcheckpack(lua_State *L){
    lua_pushlstring(L, (const char *)s_checkpack, strlen(s_checkpack) + 1);
    return 1;
}
static int
lgethandshakekey(lua_State *L){
    lua_pushlstring(L, (const char *)s_nethandshakekey, strlen(s_nethandshakekey) + 1);
    return 1;
}
...
luaopen_msgpack(lua_State *L) {
	...
	luaL_Reg l[] = {
		...
		{ "clear", lclear },
		{ "tostring", ltostring },
        { "getcheckpack", lgetcheckpack }, //add by ffy
        { "gethandshakekey", lgethandshakekey },//add by ffy
		{ NULL, NULL },
	};
	luaL_newlib(L,l);
